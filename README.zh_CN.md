...
包名 *commandos* 的灵感来自于经典策略游戏 *Commandos: Behind Enemy Lines*，但在此处，其真实意义是单词 *command* 和 *DOS* 的合体。__commandos__ 是一个轻量级的命令行分析工具，令你可以更加轻松地使用 Node.js 开发 CLI 应用程序。
...
*   参数 *cmdline* 中出现非法选项。例如，`--no-foo=bar` 即为非法选项，因为开关量是不可以赋值的；
*   参数 *cmdline* 中包含的选项与 *settings* 及 *settings.options* 中声明的选项特征相冲突。

出现异常时，如果通过 *settings.catcher* 指定了异常捕获函数，则该函数将被调用，而相应异常（`Error` 实例）将作为唯一的参数。否则，该异常将被直接抛出：
...
假如 *cmdline* 可能存在多种不同形式，我们当然可以通过多次运行 `commandos.parse()` 以正确获取命令行携带的参数。不过，通过参数 *settings.groups* 声明选项组是一个更优雅的方式。
...
注意：如果 *settings.explicit* 未明确赋值为 `true`，最好在选项组中的选项声明中合理使用 `'REQUIRED'` 关键字。下面是一个反模式：
...
一个标准的 *选项定义*（数组 *optionDefinitions* 或 *settings.options* 的元素）可以是一个对象或字符串。当它是一个对象时，可以包含以下属性：
...
*   __assignable__ *boolean* DEFAULT `true` OPTIONAL  
    若选项被声明为 __assignable__，*cmdline* 中紧随选项名之后的片断将视为该选项的值。
...
*   __nullable__ *boolean* DEFAULT `true` OPTIONAL  
    若选项的 __nullable__ 属性非真，则该选项__不应当__在 *cmdline* 中单独出现，若出现则必须附带选项值.
...
在众多使用 Node.js 编写的命令行解析工具中，[minimist](https://www.npmjs.com/package/minimist) 和 [commander](https://www.npmjs.com/package/commander) 较为知名。但是，有时候 __minimist__ 功能太过单薄，而 __commander__ 又太重，这就是我为什么设计 __commandos__ 的原因。
